...
#define OVERCLOCK_DUE 22UL //(change from 18UL for 114MHz to 15UL for 96MHz, 84MHz is 13UL or 0x0dUL)
#define MULTITASKING 0
#define FRAMESKIP 5
#define NES_ROM_FILENAME "smb.nes"
...
//(NES CONTROLLER:)
#define NESCTRL_Data       4    // The data pin for the NES controller
#define NESCTRL_Clock      3    // The clock pin for the NES controller
#define NESCTRL_Latch      2    // The latch pin for the NES controller (STROBE)

//Libraries:
#include <SchedulerARMAVR.h> 
//#include <SRAMsimple.h> //23LC1024 128kB memory ram library
#include "SPI.h"
...
ILI9341_due gfx(LCD_CS, LCD_DC, LCD_RST);
...
//********************************************************************************
uint8_t *SCREENLINE[256]; //malloc hack
uint16_t *SCREENBUFFER;

uint8_t DRAWREADY=0; 

const uint16_t _Palette[64]={
...
#define WHITE           0xFFFF
//--------------------------------------------------------------------------------
#define  NES_CLOCK_DIVIDER    12  //NCX default: 12
...
void ppu_renderscanline(int scanline, bool draw_flag){
  uint8_t *buf = SCREENLINE[scanline];
...
void ppu_scanline(int scanline, bool draw_flag){
  ...
  if (MULTITASKING==1 && draw_flag && DRAWREADY==0 && scanline < 241) {
    for (uint32_t tmp=0;tmp<256;tmp++){
      SCREENBUFFER[scanline + tmp ]=_Palette[0x3F & SCREENLINE[scanline][tmp]];
    }

    if (scanline%32==31) {
      DRAWREADY=(scanline+1)/32;
      yield();
    }  
    if (scanline==239) {
      DRAWREADY=8;
  ...
  } else if (draw_flag && scanline < 241) {
    for (uint32_t tmp=0;tmp<256;tmp++){
      SCREENBUFFER[ tmp ]=_Palette[0x3F & SCREENLINE[scanline][tmp]];

    }
    gfx.drawImage(((uint16_t*)SCREENBUFFER), 0, scanline, 256, 1);
  }
}
//--------------------------------------------------------------------------------
//APU.C
...
static void build_address_handlers(nes_t *machine) {
  ...
  if (intf->sound_ext && ENABLE_AUDIO==1) {
...
void setup() {
   ...
   if (OVERCLOCK_DUE>0) {
      //Use this (change from 18UL for 114MHz to 15UL for 96MHz, 84MHz is 13UL or 0x0dUL (CKGR_PLLAR_MULA):
      #define SYS_BOARD_PLLAR (CKGR_PLLAR_ONE | CKGR_PLLAR_MULA(OVERCLOCK_DUE) | CKGR_PLLAR_PLLACOUNT(0x3fUL) | CKGR_PLLAR_DIVA(1UL))
...
//--------------------------------------------------------------------------------
   ...
   pinMode(NESCTRL_Latch, OUTPUT);
   digitalWrite(NESCTRL_Clock, LOW);
   digitalWrite(NESCTRL_Latch, LOW);

//--------------------------------------------------------------------------------
   //LCD TFT ILI9341 init
   gfx.begin();
   gfx.setRotation(iliRotation270);  // landscape
   gfx.setFont(Arial_bold_14);
   gfx.fillScreen(ILI9341_BLACK);
   gfx.setTextColor(WHITE, BLACK);
   gfx.setTextScale(1);
   gfx.setTextLetterSpacing(2);
   gfx.println(F("NES EMU DUE by Nathalis:"));
//--------------------------------------------------------------------------------
   gfx.println(F("Initializing SD card..."));
   if (!SD.begin(SD_CS_PIN, SPI_SIXTEENTH_SPEED)) { ///SD max 5MHz!!!
      gfx.println(F("SD card initialization failed!"));
   } else {
      gfx.println(F("SD card initialization done."));
   }
//********************************************************************************
  
   NESmachine= nes_create();
   if (NULL == NESmachine) gfx.println(F("Failed to create NES instance."));
      
   gfx.println(F("Inserting cartridge."));

//********************************************************************************
   unsigned char *rom; 

   fp = SD.open(NES_ROM_FILENAME);
   rom = (unsigned char*)malloc(fp.size());

   // Read NES rom
   for (uint32_t i = 0; i < fp.size(); i++) {
      if (fp.available()) rom[i] = fp.read();
   }
//--------------------------------------------------------------------------------
   ...
   if (rom_getheader(&rom, rominfo)) goto rom_load_fail;

   // Make sure we really support the mapper 
   if (false == mmc_peek(rominfo->mapper_number)){
      gfx.print(F("Mapper not yet implemented:"));
      gfx.println(rominfo->mapper_number);
      goto rom_load_fail;
   }
   ...
   if (rom_loadrom(&rom, rominfo)) goto rom_load_fail;
...
//********************************************************************************
   ...
   if (ENABLE_AUDIO==1) apu_setext(NESmachine->apu, NESmachine->mmc->intf->sound_ext);
...
inscart_fail:
   gfx.println(F("Insert Cartridge Fail"));
   exit(0);
  
inscart_success:      
   gfx.println(F("Insert Cartridge OK."));

   if (MULTITASKING==1) {
      Scheduler.startLoop(VideoLoop);
      gfx.println(F("Install Timmer done"));
   }    

   //malloc hack...
   for (uint16_t tmp=0;tmp<256;tmp++) SCREENLINE[tmp]=(uint8_t*)malloc(1);
   SCREENBUFFER=(uint16_t*)malloc(256*1);
   gfx.println(F("Screen Memory Malloc done."));

   delay(500);
   
   uint8_t tmp=0;
   while (true) {
      if (FRAMESKIP==0) nes_renderframe(true);
      else if (tmp%(FRAMESKIP)==0) nes_renderframe(true);
      else nes_renderframe(false);
      tmp++; 
   } 
}
...
void VideoLoop() {
   if (DRAWREADY>0) {
      gfx.drawImage(((uint16_t*)SCREENBUFFER), 0, DRAWREADY, 256, 1);
      DRAWREADY=0;
   }
   Scheduler.delay(3);
}
